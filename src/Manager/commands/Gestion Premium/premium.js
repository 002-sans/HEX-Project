        ...
        switch(interaction.options.getSubcommand())
        {
            ...
            case 'list':
                ...
                const embed = {
                    ...
                    description: keys.map(key => `\`${key}\` - Expire <t:${isNaN(codes[key].expiresAt) ? codes[key].expiresAt : Math.round(codes[key].expiresAt / 1000)}:R> ${codes[key].used ? `- <@${codes[key].by}>` : ''}`).join('\n'),
                    color: 0x000000,
                }
